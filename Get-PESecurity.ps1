<#
  # Original Authors: Eric Gruber 2014, NetSPI
  # Alex Verboon July 28.2017, added Control Flow Guard information

  # Module converted from .psm1 to .ps1
  # All credits go to their original authors
  # Options to run it all in one go
...
function Get-PESecurity
{
  ...
  Begin
  {
    # Original module content starts here (enumeration, structs, etc.)
    # ... (content omitted for brevity, but all your code is included below)
    $ModuleName = 'Win32'
    ...
    $ImageDllCharacteristics = enumerate $Mod PE.IMAGE_DLLCHARACTERISTICS UInt16 @{
      ...
      NO_SEH = 0x0400 # Image does not use SEH.
      # No SE handler may reside in this image
    ...
    # Original module content ends here
  }
  Process
  {
    $Files = Get-Files -Directory $Directory -File $File -Recursive:$Recursive
	Enumerate-Files $Files $Table
	#Write-Host "DEBUG: Table rows generated: $($Table.Rows.Count)" -ForegroundColor Cyan
...
function Get-Files
{
  ...
  # Use a single pipeline structure to collect and flatten all results
  $AllFiles = @(
      if($Directory)
      {
          Get-FilesFromDirectory -Directory $Directory -Recursive:$Recursive
      }
      if($File)
      {
          (Get-Item $File -ErrorAction SilentlyContinue -Force 2>$null).FullName
      }
  )
...
function Get-FilesFromDirectory
{
  ...
  $PathToScan = Join-Path -Path $Directory -ChildPath '*'
  
  # Use direct assignment to capture file paths as an array of strings
  if($Recursive)
  {
    $Files = Get-ChildItem -Path $PathToScan -Recurse -Include *.exe, *.dll -ErrorAction SilentlyContinue -Force 2>$null | 
             Select-Object -ExpandProperty FullName
  }
  else
  {
    $Files = Get-ChildItem -Path $PathToScan -Include *.exe, *.dll -ErrorAction SilentlyContinue -Force 2>$null | 
             Select-Object -ExpandProperty FullName
  }
...
function enumerate
{
  <#
    ...
    The in-memory module that will host the enum.
    Use
    ...
    Sorry, I'm not going to name it
    New-Enum.
    :P
  #>
...
function struct
{
  <#
    ...
    The in-memory module that will host the struct.
    Use
    New-InMemoryModule to define an in-memory module.
    .PARAMETER FullName
    ...
    A hashtable of fields.
    Use the 'field' helper function to ease
    defining each field.
    ...
    # Example of
    using an explicit layout in order to create a union.
...
function Add-Win32Type
{
  <#
    ...
    Win32 unmanaged) functions in PowerShell.
    After providing
    ...
    The in-memory module that will host the functions.
    Use
    New-InMemoryModule to define an in-memory module.
    .PARAMETER Namespace

    An optional namespace to prepend to the type.
    Add-Win32Type defaults
    ...
    $Types = $FunctionDefinitions |
    Add-Win32Type -Module $Mod -Namespace 'Win32'
    ...
    Add-Win32Type with an array of function signatures.
    That way, they
...
# --- END OF ORIGINAL MODULE CODE ---

# Calls the Get-PESecurity function

# MANUAL MODE: Uncomment and modify these lines if command-line arguments are not working.
# 
# $ManualMode = $true
# $Manual_Directory = "C:\Program Files (x86)\Microsoft\"
# $Manual_File = ""
# $Manual_Recursive = $true
# $Manual_SkipAuthenticode = $false

try {
    # 1. Check for Manual Mode: If $ManualMode is set to $true, use the manual variables.
    if ($ManualMode -eq $true) {
        Write-Host "--- Executing in MANUAL MODE ---" -ForegroundColor Red

        if ($Manual_File) {
            # Run single file scan
            Get-PESecurity -File $Manual_File -SkipAuthenticode:$Manual_SkipAuthenticode
        } elseif ($Manual_Directory) {
            # Run directory scan (respecting recursive flag)
            Get-PESecurity -Directory $Manual_Directory -Recursive:$Manual_Recursive -SkipAuthenticode:$Manual_SkipAuthenticode
        } else {
            Write-Host "Manual mode active but no target specified." -ForegroundColor Yellow
        }
    }
    # 2. Normal execution (uses command-line arguments)
    elseif ($args) {
        Get-PESecurity @Args
    }
    else {
        Write-Host "No parameters provided. Please run with -File or -Directory."
    }
}
catch {
    Write-Host "`n[!! CRITICAL ERROR !!] Script Terminated Silently.`n" -ForegroundColor Red
...
# To run this script, uncomment one of the following lines below.
# Uses:

# 1. Example to scan a single file:
# Get-PESecurity.ps1 -File C:\Windows\System32\kernel32.dll

# 2. Example to recursively scan a directory:
# Get-PESecurity -Directory C:\Windows\System32\ -Recursive

# 3. Example bypass Powershell restrictions
# powershell.exe -ExecutionPolicy Bypass -File ".\Get-PESecurity.ps1" -File "C:\Program Files (x86)\Microsoft\"

# 4. Example to export as CSV
